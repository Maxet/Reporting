<?php
	...
	function user_names( $type = 'username' ) {
		...
		foreach ( $result as $row ) {
			...
			if ( $type == 'realname' ) {
				...
				else                        { $tmp = lang_get( 'plugin_Reporting_unknown' ); }
				} else {
	...
	$granularities = array(
    1 => lang_get( 'plugin_Reporting_daily' ), 2 => lang_get( 'plugin_Reporting_weekly' ),
    3 => lang_get( 'plugin_Reporting_monthly' ), 4 => lang_get( 'plugin_Reporting_yearly' )
	);
	...
	$reports_arr = array(
    'synthese_detail'				=> lang_get( 'plugin_Reporting_synthese_detail' ),
	'date_history'					=> lang_get( 'plugin_Reporting_date_history'),
	'test_mgo'						=> lang_get( 'plugin_Reporting_test_mgo')
	);
	...
	if ( isset( $_REQUEST['page'] ) and !empty( $_REQUEST['page'] ) ) {
		...
		$current_page = substr( $current_page, strlen( lang_get( 'plugin_Reporting_title' ) ) + 1 );
	}
	...
	$whichReport = "<select id='reporting'>\n";
	
	foreach ( $reportsToShow as $key => $val ) {
		if ( $val == $current_page ) { $selected = ' selected '; } else { $selected = ''; }
		$whichReport .= "<option " . $selected . " value='./plugin.php?page=Reporting/" . $val . "'>" . $reports_arr[$val] . "</option>\n";
	}
	...
	$startDateInputFilter_arr = array (
		1 => lang_get( 'plugin_Reporting_start_date_option1' ),
		2 => lang_get( 'plugin_Reporting_start_date_option2' ),
		3 => lang_get( 'plugin_Reporting_start_date_option3' ),
		4 => lang_get( 'plugin_Reporting_start_date_option4' ),
		5 => lang_get( 'plugin_Reporting_start_date_option5' ),
	);
	...
	$dt_language_snippet = '
		...
		"language": {
		"processing":     "' . lang_get( 'plugin_Reporting_dt_processing' ) . '",
		"info":           "' . lang_get( 'plugin_Reporting_dt_info' ) . '",
		"infoEmpty":      "' . lang_get( 'plugin_Reporting_dt_infoEmpty' ) . '",
		"infoPostFix":    "' . lang_get( 'plugin_Reporting_dt_infoPostFix' ) . '",
		"loadingRecords": "' . lang_get( 'plugin_Reporting_dt_loadingRecords' ) . '",
		"zeroRecords":    "' . lang_get( 'plugin_Reporting_dt_zeroRecords' ) . '",
		"emptyTable":     "' . lang_get( 'plugin_Reporting_dt_emptyTable' ) . '",
		"paginate": {
		"first":      "' . lang_get( 'plugin_Reporting_dt_first' ) . '",
		"previous":   "' . lang_get( 'plugin_Reporting_dt_previous' ) . '",
		"next":       "' . lang_get( 'plugin_Reporting_dt_next' ) . '",
		"last":       "' . lang_get( 'plugin_Reporting_dt_last' ) . '"
		},
		"aria": {
		"sortAscending":  "' . lang_get( 'plugin_Reporting_dt_sortAscending' ) . '",
		"sortDescending": "' . lang_get( 'plugin_Reporting_dt_sortDescending' ) . '"
		}
		}
