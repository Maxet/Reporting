<?php
	...
	class ReportingPlugin extends MantisPlugin {
		# Plugin definition
		function register() {
			$this->name         = lang_get( 'plugin_Reporting_title' );
			$this->description  = lang_get ( 'plugin_Reporting_description' );
		...
		function showreport_menu() {
			if ( access_has_global_level( plugin_config_get( 'access_threshold' ) ) ) {
				return array(
					array( 
						'title'         => lang_get( 'plugin_Reporting_title' ),
						'access_level'  => plugin_config_get( 'access_threshold' ),
						'url'           => 'plugin.php?page=Reporting/start_page',
						'icon'          => 'fa fa-database'
					),
				);
			}
